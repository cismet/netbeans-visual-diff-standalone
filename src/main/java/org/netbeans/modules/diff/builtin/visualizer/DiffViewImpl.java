 ...
 */
public class DiffViewImpl extends javax.swing.JPanel implements org.netbeans.api.diff.DiffView,
    ...
    /**
     * Creates a new DiffViewImpl object.
     */
    public DiffViewImpl() {
    }
     ...
     */
    public DiffViewImpl(final StreamSource ss1, final StreamSource ss2) throws IOException {
        ...
        final Reader r1 = ss1.createReader();
        final Reader r2 = ss2.createReader();
        final String title1 = ss1.getTitle();
        final String title2 = ss2.getTitle();
        ...
        jEditorPane1.getAccessibleContext()
                .setAccessibleName(org.openide.util.NbBundle.getMessage(DiffViewImpl.class, "ACS_EditorPane1A11yName")); // NOI18N
        jEditorPane1.getAccessibleContext()
        ...
        jEditorPane2.getAccessibleContext()
                .setAccessibleName(org.openide.util.NbBundle.getMessage(DiffViewImpl.class, "ACS_EditorPane2A11yName")); // NOI18N
        jEditorPane2.getAccessibleContext()
                .setAccessibleDescription(org.openide.util.NbBundle.getMessage(
                        DiffViewImpl.class,
        ...
        if ((r1 != null) && (r2 != null)) {
            final DiffProvider provider = DiffModuleConfig.getDefault().getDefaultDiffProvider();
...
//                at org.netbeans.modules.versioning.system.cvss.ui.actions.diff.DiffMainPanel$DiffPrepareTask.run(DiffMainPanel.java:634)
        ...
        try {
            final Runnable awtTask = new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            if (source1 != null) {
                                setSource1(new StringReader(source1));
                            }
                            if (source2 != null) {
                                setSource2(new StringReader(source2));
                            }
            ...
            final ErrorManager err = ErrorManager.getDefault();
            err.notify(e);
     ...
     * @throws  IOException  DOCUMENT ME!
     */
    private static void copyStreamsCloseAll(final Writer out, final Reader in) throws IOException {
     ...
     * @throws  IOException  DOCUMENT ME!
     */
    private void saveSources(final Reader r1, final Reader r2) throws IOException {
        if (r1 != null) {
    ...
    public void setCurrentDifference(final int diffNo) throws UnsupportedOperationException {
        if ((diffNo < -1) || (diffNo >= diffs.length)) {
    ...
    public int getCurrentDifference() throws UnsupportedOperationException {
        ...
        if (jViewport1 != null) {
            final int viewHeight = jViewport1.getViewSize().height;
            final java.awt.Point p1;
            initGlobalSizes();              // The window might be resized in the mean time.
            p1 = jViewport1.getViewPosition();
            final int HALFLINE_CEILING = 2; // compensation for rounding error and partially visible lines
            final float firstPct = ((float)p1.y / (float)viewHeight);
            firstVisibleLine = (int)(firstPct * totalLines) + HALFLINE_CEILING;
            final float lastPct = ((float)(jViewport1.getHeight() + p1.y) / (float)viewHeight);
            lastVisibleLine = (int)(lastPct * totalLines) - HALFLINE_CEILING;

            for (int i = 0; i < diffs.length; i++) {
                final int startLine = diffShifts[i][0] + diffs[i].getFirstStart();
                final int endLine = diffShifts[i][0] + diffs[i].getFirstEnd(); // there no add changes in left pane
                if (((firstVisibleLine < startLine) && (startLine < lastVisibleLine))
                    ...
                    if (i == currentDiffLine) {
                        return currentDiffLine;                                // current is visible, eliminate hazards
                                                                               // use it.
                    }
                    candidate = i;                                             // takes last visible, optimalized for
                                                                               // Next>
                }
            }
        }
    ...
    public void addNotify() {
        ...
        final List<Action> actions = new ArrayList<Action>(2);
    ...
    private void insertEmptyLinesNotReported() {
        final String docText1 = getDocumentText1();
        final String docText2 = getDocumentText2();
        final int[] begin1 = { 0 };
        final int[] end1 = { -1 };
        final int[] begin2 = { 0 };
        final int[] end2 = { -1 };
