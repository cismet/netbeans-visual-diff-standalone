...
public class DiffComponent extends org.openide.windows.TopComponent {
     ...
     * @param  mainTitle    DOCUMENT ME!
     * @param  mimeType     DOCUMENT ME!
     * @param  sourceName1  DOCUMENT ME!
     * @param  sourceName2  DOCUMENT ME!
     * @param  title1       DOCUMENT ME!
     * @param  title2       DOCUMENT ME!
     * @param  r1           DOCUMENT ME!
     * @param  r2           DOCUMENT ME!
     */
    public DiffComponent(final Difference[] diffs,
            final String mainTitle,
            final String mimeType,
            final String sourceName1,
            final String sourceName2,
            final String title1,
            final String title2,
            final Reader r1,
            final Reader r2) {
    ...
    public DiffComponent(final Difference[] diffs,
        ...
        diffPanel.addNextLineButtonListener(new java.awt.event.ActionListener() {
                ...
                public void actionPerformed(final java.awt.event.ActionEvent evt) {
                    ...
                    currentDiffLine++;
                    if (currentDiffLine >= diffs.length) {
                        currentDiffLine = 0;
                    }
                    showCurrentLine();
                }
...
//        initComponents ();
    ...
    private void showCurrentLine() {
        ...
        final Difference diff = diffs[currentDiffLine];
        int line = diff.getFirstStart() + diffShifts[currentDiffLine][0];
        if (diff.getType() == Difference.ADD) {
            line++;
        }
        final int lf1 = diff.getFirstEnd() - diff.getFirstStart() + 1;
        final int lf2 = diff.getSecondEnd() - diff.getSecondStart() + 1;
        final int length = Math.max(lf1, lf2);
     ...
     * @param  r2           DOCUMENT ME!
     */
    private void initContent(final String mimeType,
            final String sourceName1,
            final String sourceName2,
            final String title1,
            final String title2,
    ...
    @Override
    @Deprecated
    public boolean canClose(final Workspace workspace, final boolean last) {
        final boolean can = super.canClose(workspace, last);
        if (last && can) {
            exitForm(null);
        }
        return can;
    ...
    public void writeExternal(final ObjectOutput out) throws IOException {
        ...
        /*
         * } catch (IOException exc) { System.out.println("exc = "+exc); exc.printStackTrace(); throw exc; }
         */
    }
     ...
     * DOCUMENT ME!
     *
     * @param  updateActionLines  DOCUMENT ME!
     */
    private void insertEmptyLines(final boolean updateActionLines) {
        final int n = diffs.length;
        // int ins1 = 0;
        // int ins2 = 0;
        // D.deb("insertEmptyLines():"); // NOI18N
        for (int i = 0; i < n; i++) {
            final Difference action = diffs[i];
            final int n1 = action.getFirstStart() + diffShifts[i][0];
            final int n2 = action.getFirstEnd() + diffShifts[i][0];
            final int n3 = action.getSecondStart() + diffShifts[i][1];
            final int n4 = action.getSecondEnd() + diffShifts[i][1];
     ...
     */
    private void setDiffHighlight(final boolean set) {
        final int n = diffs.length;
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (action.getType()) {
                case Difference.DELETE: {
                    if (set) {
                        highlightRegion1(n1, n2, colorMissing);
                    } else {
                ...
                case Difference.ADD: {
                    if (set) {
                        highlightRegion2(n3, n4, colorAdded);
                    } else {
    ...
    /**
     * Read one line from the given text, from the given position. It returns the end position of this line and the
     * beginning of the next one.
     *
     * @param   begin  Contains just one value - IN: the beginning of the line to read. OUT: the start of the next line.
     * @param   end    Contains just one value - OUT: the end of the line.
     * @param   text   The text to read.
     *
     * @return  The line.
     */
    private static String readLine(final int[] begin, final int[] end, final String text) {
        ...
        final String line = text.substring(begin[0], end[0]);
        begin[0] = end[0] + 1;
     ...
     * @param   diffIndex   DOCUMENT ME!
     * @param   diffs       DOCUMENT ME!
     * @param   diffShifts  DOCUMENT ME!
     *
     * @return  The index of the desired difference in the supplied array or a value, that is bigger then the array size
     *          if such a diff does not exist.
     */
    private static int findDiffForLine(final int lineNumber,
            int diffIndex,
            final Difference[] diffs,
            final int[][] diffShifts) {
        while (diffIndex < diffs.length) {
            if (((diffs[diffIndex].getFirstEnd() + diffShifts[diffIndex][0]) >= lineNumber)
                        || ((diffs[diffIndex].getSecondEnd() + diffShifts[diffIndex][1]) >= lineNumber)) {
                break;
            }
            diffIndex++;
        }
     ...
     */
    private static boolean isLineInDiff(final int lineNumber, final Difference diff, final int[] diffShifts) {
        final int l1 = diff.getFirstStart() + diffShifts[0];
        final int l2 = diff.getFirstEnd() + diffShifts[0];
        final int l3 = diff.getSecondStart() + diffShifts[1];
        final int l4 = diff.getSecondEnd() + diffShifts[1];
     ...
     * DOCUMENT ME!
     *
     * @param   beginLine  DOCUMENT ME!
     * @param   text       DOCUMENT ME!
     * @param   endLine    DOCUMENT ME!
     *
     * @return  DOCUMENT ME!
     */
    private static int numEmptyLines(final int beginLine, final String text, final int endLine) {
        if ((endLine >= 0) && (endLine <= beginLine)) {
            return 0;
        }
        int numLines = 0;
        final int[] begin = { beginLine };
        final int[] end = { 0 };
        do {
            final String line = readLine(begin, end, text);
    ...
    /**
     * We have to keep the balance of lines from the first and the second document, so that corresponding lines will
     * have the same line number in the document. Because some diff providers can be set not to report changes in empty
     * lines, we have to use heuristics to balance the corresponding lines manually if they do not match.
     *
     * <p>This method goes through both documents and finds unreported differences in empty lines. Whenever it
     * encounters an empty line with a corresponding non-empty line, which in not inside a difference (== unreported),
     * it checks whether the following lines match (because there can be unreported difference in the amount of space
     * rather than a missing or added line). If following lines "match", then silently add an empty line to the other
     * document. This added line is not highlighted, since it was not reported as a difference.</p>
     */
    private void insertEmptyLinesNotReported() {
        final String docText1 = diffPanel.getDocumentText1();
        final String docText2 = diffPanel.getDocumentText2();
        final int[] begin1 = { 0 };
        final int[] end1 = { -1 };
        final int[] begin2 = { 0 };
        final int[] end2 = { -1 };
        final int n1 = docText1.length();
        final int n2 = docText2.length();
        int lineNumber = 1;
        int diffIndex = 0;
        do {
            final int lastBegin1 = begin1[0];
            final int lastBegin2 = begin2[0];
            final String line1 = readLine(begin1, end1, docText1);
            final String line2 = readLine(begin2, end2, docText2);
            if ((line1.length() == 0) && (line2.length() > 0)) {
                ...
                if ((diffIndex >= diffs.length) || !isLineInDiff(lineNumber, diffs[diffIndex], diffShifts[diffIndex])) {
                    ...
                    if (line2.trim().length() == 0) {
                        final int emptyLines1 = numEmptyLines(
                                begin1[0],
                                docText1,
                                (diffIndex < diffs.length) ? diffs[diffIndex].getFirstStart() : -1);
                        final int emptyLines2 = numEmptyLines(
                                begin2[0],
                                docText2,
                                (diffIndex < diffs.length) ? diffs[diffIndex].getSecondStart() : -1);
     ...
     */
    private void shiftDiffs(final boolean inFirstDoc, final int fromLine) {
        final int n = diffs.length;
        for (int i = 0; i < n; i++) {
            final Difference action = diffs[i];
            if (inFirstDoc) {
     ...
     * @return  DOCUMENT ME!
     *
     * @throws  java.io.ObjectStreamException  DOCUMENT ME!
     */
    @Override
    ...
    private void exitForm(final java.awt.event.WindowEvent evt) {
         ...
         * lineNumbersVisible); } catch (Throwable exc) { // editor module not found }
         */
        ...
        // dispose ();
        /*
         * for(Iterator it = closeListeners.iterator(); it.hasNext(); ) { ((TopComponentCloseListener)
         * it.next()).closing(); }
         */
    }
}
