...
public class ExportPatch {
    ...
    /**
     * Creates a new ExportPatch object.
     */
    private ExportPatch() {
    }
    ...
    public static void exportPatch(final StreamSource[] base, final StreamSource[] modified) {
        ...
        final FileFilter[] old = chooser.getChoosableFileFilters();
        for (int i = 0; i < old.length; i++) {
            final FileFilter fileFilter = old[i];
            chooser.removeChoosableFileFilter(fileFilter);
        }
        chooser.setCurrentDirectory(new File(
                DiffModuleConfig.getDefault().getPreferences().get(
                    "ExportDiff.saveFolder",
        ...
        final DialogDescriptor dd = new DialogDescriptor(
                chooser,
                NbBundle.getMessage(ExportPatch.class, "CTL_Export_Title"));
                    ...
                    if (state.equals(JFileChooser.APPROVE_SELECTION)) {
                        ...
                        if (destination.exists()) {
                            final NotifyDescriptor nd = new NotifyDescriptor.Confirmation(
                                    NbBundle.getMessage(ExportPatch.class, "BK3005", destination.getAbsolutePath()));
                        ...
                        DiffModuleConfig.getDefault()
                                .getPreferences()
                                .put("ExportDiff.saveFolder", destination.getParent());
     ...
     */
    private static void exportDiff(final StreamSource[] base,
            final StreamSource[] modified,
        ...
        try {
            ...
            out.write(
                ("# This patch can be applied using context Tools: Apply Diff Patch action on respective folder." + sep)
            ...
            if (success) {
                StatusDisplayer.getDefault()
                        .setStatusText(NbBundle.getMessage(ExportPatch.class, "BK3004", new Integer(exportedFiles)));
                if (exportedFiles == 0) {
                    destination.delete();
                } else {
    ...
    private static void exportDiff(final StreamSource base,
            ...
            final FileFilter format) throws IOException {
        ...
        try {
            ...
            final TextDiffVisualizer.TextDiffInfo info = new TextDiffVisualizer.TextDiffInfo(
    ...
    private static void openFile(final File file) {
        ...
        if (fo != null) {
            try {
                ...
                final OpenCookie oc = dao.getCookie(OpenCookie.class);
                if (oc != null) {
                    oc.open();
                }
